...
def _parse_tree(node_id: int,
    ...
    Args:
        node_id: Current nodo ID
        parent: Parent nodo ID
    ...
    # Set nodo name based on parent split
    if parent == 'null':
        nodo['name'] = "HEAD"
    else:
        ...
        if feature in binary_features:
            nodo['name'] = f"{feature}: {'0' if pos == 'left' else '1'}"
        else:
            comparison = "<=" if pos == 'left' else ">"
            threshold_val = int(threshold) if feature in int_features else round(threshold, 3)
            nodo['name'] = f"{feature} {comparison} {threshold_val}"

    # Set nodo attributes
    nodo['parent'] = int(parent) if isinstance(parent, (int, float)) else parent
    nodo['self'] = int(node_id)
    nodo['sample'] = int(tree_model.tree_.n_node_samples[node_id])
    nodo['impurity'] = round(tree_model.tree_.impurity[node_id], 3)
    nodo['value'] = [int(v) for v in tree_model.tree_.value[node_id][0]]
    nodo['predict'] = target_names[np.argmax(nodo['value'])]
    nodo['color'] = tree_info['target_colors'][np.argmax(nodo['value'])]
    nodo['pos'] = pos
    ...
    if left_child != -1 or right_child != -1:
        nodo['children'] = []
        if left_child != -1:
            nodo['children'].append(_parse_tree(left_child, node_id, 'left', tree_info))
        if right_child != -1:
            nodo['children'].append(_parse_tree(right_child, node_id, 'right', tree_info))
...
def _extract_rules(node_id: int,
    ...
    """Extract decision rules for each tree nodo recursively.
...
def _clean_rules(tree_rules: Dict[int, Dict],
    ...
    Returns:
        Dictionary mapping nodo IDs to formatted rule strings
    """
    tree_rules_clean = {}
    
    for node_id, nodo in tree_rules.items():
        rules = []
        
        for feature, bounds in nodo['features'].items():
