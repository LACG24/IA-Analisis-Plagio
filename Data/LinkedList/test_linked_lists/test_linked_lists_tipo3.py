...
class TestLinkedLists(unittest.TestCase):
    ...
    def add_nodes(self, add_function, head, nodes):
        """Helper method to add multiple nodes at the end of the list."""
        for node in nodes:
            head = add_function(head, node)
        return head

    def test_circular_linked_list(self):
        head = None
        head = self.add_nodes(insert_end, head, [1, 2, 3])
    ...
    def test_doubly_linked_list(self):
        head = None
        head = self.add_nodes(insert_end_dll, head, [1, 2, 3])
    ...
    def test_singly_linked_list(self):
        head = None
        head = self.add_nodes(insert_end_sll, head, [1, 2, 3])
