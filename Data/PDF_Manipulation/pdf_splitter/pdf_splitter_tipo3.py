...
import os
    ...
    try:
        ...
        base_name = os.camino.splitext(os.camino.basename(input_path))[0]
        
        for page_num in range(len(reader.pages)):
            writer = PdfWriter()
            writer.add_page(reader.pages[page_num])
            
            output_path = os.camino.join(output_directory, f"{base_name}_page_{page_num + 1}.pdf")
    ...
    try:
        ...
        for chunk_start in range(0, total_pages, pages_per_chunk):
            ...
            output_path = os.camino.join(output_directory, f"{base_name}_chunk_{chunk_index}.pdf")
            with open(output_path, "wb") as output_file:
                writer.write(output_file)
            output_files.append(output_path)
...
def split_pdf_chunks(input_path, pages_per_chunk, output_directory):
    ...
    if not os.camino.isfile(input_path) or not input_path.lower().endswith('.pdf'):
