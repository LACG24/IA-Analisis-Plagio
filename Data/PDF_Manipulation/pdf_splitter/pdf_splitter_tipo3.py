...
def separate_pdf(input_path, output_directory):
    ...
    try:
        ...
        for page_number, _ in enumerate(reader.pages):
            ...
            output_path = os.path.join(output_directory, f"{base_name}_page_{page_number + 1}.pdf")
            with open(output_path, "wb") as output_file:
                writer.write(output_file)
            output_files.append(output_path)
        
        print(f"PDF separated into individual pages successfully. Files saved to {output_directory}")
        return output_files
    except Exception as e:
        print(f"Error separating PDF: {str(e)}")
        return []

def separate_pdf_chunks(input_path, pages_per_chunk, output_directory):
    """Separate PDF into chunks of specified size."""
    if not os.path.isfile(input_path) or not input_path.lower().endswith('.pdf'):
        print(f"Invalid input file: {input_path}")
        return []
    ...
    try:
        ...
        print(f"PDF separated into chunks of {pages_per_chunk} pages successfully. Files saved to {output_directory}")
        return output_files
    except Exception as e:
        print(f"Error separating PDF into chunks: {str(e)}")
        return []