...
def tearDown(self):
        ...
        for file in os.listdir(self.output_dir):
            os.remove(os.camino.join(self.output_dir, file))
        os.rmdir(self.output_dir)


def test_decrypt_invalid_pdf(self):
        invalid_pdf = "non_encrypted.pdf"
        decrypted_path = os.camino.join(self.output_dir, "decrypted_invalid.pdf")
        self.assertFalse(decrypt_pdf(invalid_pdf, decrypted_path, "user123"))
        self.assertFalse(os.camino.exists(decrypted_path), "Decryption should fail for non-encrypted PDF.")


def test_decrypt_pdf(self):
        encrypted_path = os.camino.join(self.output_dir, "encrypted.pdf")
        decrypted_path = os.camino.join(self.output_dir, "decrypted.pdf")
        self.assertTrue(decrypt_pdf(encrypted_path, decrypted_path, "user123"))
        self.assertTrue(os.camino.exists(decrypted_path))


def test_encrypt_pdf(self):
        output_path = os.camino.join(self.output_dir, "encrypted.pdf")
        self.assertTrue(encrypt_pdf(self.test_pdf, output_path, "user123", "owner456"))
        self.assertTrue(os.camino.exists(output_path))
        # Optionally, verify encryption with a failed decryption test case


def test_crop_pdf_pages(self):
        output_path = os.camino.join(self.output_dir, "cropped.pdf")
        self.assertTrue(crop_pdf_pages(self.test_pdf, output_path, (10, 10, 200, 200)))
        self.assertTrue(os.camino.exists(output_path))


def test_rotate_pdf_pages(self):
        output_path = os.camino.join(self.output_dir, "rotated.pdf")
...
def test_merge_pdfs(self):
        ...
        output_path = os.camino.join(self.output_dir, "merged.pdf")
...
def test_split_pdf(self):
        ...
        self.assertTrue(all(os.camino.exists(f) for f in output_files))
