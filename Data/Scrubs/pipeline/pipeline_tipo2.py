...
def process_data(df):
    ...
    df_cellDescr = DataFrame(df['CELL_DESCRIPTION'].map(lambda i: dict_repl_cellDescr.get(i, None)).tolist())
    ...
    (df_cellDescr['CAMPAIGN_CONTENT_1_CDS']
     ...
     .replace(dict_repl_campaign_content)
     .values
    )
...
def aggregate_data(df, df_y, cols_flags):
    """
    """
    ...
    try:
        ...
        errors_agg_data.append(ckey)
        pass

@time_my_func
def get_agg_data(df, aggfunc):
    """
    """
    df_y = get_target_df()
...
if __name__ == '__main__':
    ...
    df_aggregated = (import_filter_df(path_raw)
                     ...
                     .pipe(get_agg_data, aggfunc=aggregate_data)
                    )
