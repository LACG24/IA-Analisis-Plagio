...
class TestRandomForest(unittest.TestCase):
    ...
    def setUp(self):
        self.X_data = np.array([[2], [3], [10], [19], [11], [18], [23], [45]])
        self.y_data = np.array([0, 0, 1, 1, 1, 1, 1, 1])
        self.rf_model = RandomForest(n_trees=5, max_depth=3)

    def test_fit_predict(self):
        self.rf_model.fit(self.X_data, self.y_data)
        predictions = self.rf_model.predict(np.array([[5], [15], [25]]))
