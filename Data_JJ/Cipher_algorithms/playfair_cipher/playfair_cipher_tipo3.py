...
def preprocess_message(message):
    ...
    """
    message = message.upper().replace('J', REPLACE_CHAR)
    processed_message = ""
    i = 0
    while i < len(message):
...
def locate_position(character, key_table):
    """
    Finds the position of a character in the key table.
    
    Parameters:
    - character (str): The character to locate in the table.
...
def encrypt_playfair(plaintext, key):
    ...
    while i < len(plaintext):
        row1, col1 = locate_position(plaintext[i], key_table)
        row2, col2 = locate_position(plaintext[i + 1], key_table)
        if row1 == row2:
...
def decrypt_playfair(ciphertext, key):
    ...
    while i < len(ciphertext):
        row1, col1 = locate_position(ciphertext[i], key_table)
        row2, col2 = locate_position(ciphertext[i + 1], key_table)
        if row1 == row2:
            # Same row: shift left
